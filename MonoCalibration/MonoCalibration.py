...
images = glob.glob('./StereoCalibration/images/LEFT/*.jpg')
for fname in images:
...
# 去畸变
img2 = cv2.imread('./StereoCalibration/images/LEFT/12 baseline = 8.jpg')
h, w = img2.shape[:2]
...
dst = dst[y:y + h, x:x + w]
cv2.imshow('fin', dst)
cv2.imwrite('./MonoCalibrate/fin.png', dst)
cv2.waitKey()
cv2.destroyAllWindows()
